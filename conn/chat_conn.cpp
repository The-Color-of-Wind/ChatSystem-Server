...
void chat_conn::init()
{
	...
	m_start_line = 0;	//m_read_buf中已经解析的字符个数
	...
	//receiveResponseJsonArray = Json::arrayValue;

	//sendResponseJsonArray = Json::arrayValue;
	//if(!receiveResponseJsonVector.size())
		//receiveResponseJsonVector.clear();
}

//读取客户端发来的全部数据，并将数据转变为Json
bool chat_conn::read_once()
{
	...
	//cout << "read_once() : : :m_read_buf = " << m_read_buf << endl;
...
#endif
	/*
	// 解析 HTTP 头部，提取 JSON 数据部分
	string http_request(m_read_buf, m_read_idx);
	size_t json_start = http_request.find("\r\n\r\n"); // 找到 JSON 部分的起始位置
	if (json_start == string::npos) {
		cerr << "Invalid HTTP request: No JSON found" << endl;
		return false;
	}
	...
	string json_body = http_request.substr(json_start); // 提取 JSON 数据部分
	*/
	Json::CharReaderBuilder reader;

	string errs;
	istringstream ss(m_read_buf);
	jsonData = Json::Value();
	...
	while (bytes_sent < m_write_idx) {
		...
		if (temp == -1) {
			if (errno == EAGAIN || errno == EWOULDBLOCK) {
				...
				perror("Connection reset or closed");
				return false;
			}
			else {
...
void chat_conn::process()
{
	...
	CHAT_CODE read_ret = process_read();	//解析报文，数据在m_read_buf中
...
string chat_conn::getMysql_UserResult(const string &type, const string &sqlStatement)
{
	...
	if (mysql_num_rows(result) == 0) {
		...
		while (MYSQL_ROW row = mysql_fetch_row(result)) {
			...
			Server::instance()->addUserSocket(row[0], m_sockfd);
...
string chat_conn::sendMessage_Mysql(const string &type, string &sqlStatement)
{
	...
	int socketFd = Server::instance()->getSocketByUserId(receiver_id);
...
{
	Server::instance()->removeUserSocket(this->user->getUserId());
